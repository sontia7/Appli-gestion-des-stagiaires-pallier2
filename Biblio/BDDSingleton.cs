...
namespace Biblio
{
    public class BDDSingleton
    {
        ...
        private BDDStagiaires BDDStagiaire { get; set; }

        //Propriété renvoyant un booléen informant s'il y a des modifications en attente de sauvegarde ou non dans la BDD.
        public bool ModificationsEnAttenteStagiaire => BDDStagiaire?.ChangeTracker.HasChanges() ?? false;
        ...
        public ReadOnlyObservableCollection<Stagiaire> Stagiaires { get; private set; }
        ...
        public Stagiaire AjouterStagiaire(string matricule, string login, string motdepasse, string cni, string adresse, string etablissement, bool niveau, string lieu, Villes ville, string theme, Domaines domaine, DateTime dateDebut, DateTime dateFin, int note) { 
            return BDDStagiaire?.AjouterStagiaire(
                matricule,
                login,
                motdepasse,
                cni,
        ...
        public void SupprimerStagiaire(Stagiaire stagiaire) { BDDStagiaire?.SupprimerStagiaire(stagiaire); }
        #endregion
        ...
        public void ChargerDonneesStagiaire(string nomFichier = "", bool forcageNouveauFichier = false)
        {
            //Création de l'objet BDD (appel de l'un ou l'autre constructeur en fonction qu'il y ait un nom de fichier spécifié ou non)
            if (nomFichier == string.Empty) { BDDStagiaire = new BDDStagiaires(); }
            else { BDDStagiaire = new BDDStagiaires(nomFichier); }

            if (forcageNouveauFichier) { BDDStagiaire.Database.EnsureDeleted(); } //S'il faut créer une nouvelle base de données, d'abord suppression de celle qui existe (s'il y en a une).
            BDDStagiaire.Database.EnsureCreated(); //Vérification de l'existence de la BDD. Si non, création de celle-ci.
            ...
            BDDStagiaire.Stagiaires.Load(); //Chargement de la table Personnes.
            ...
            Stagiaires = new ReadOnlyObservableCollection<Stagiaire>(BDDStagiaire?.Stagiaires.Local.ToObservableCollection());
        }

        public void SauvegarderModificationsStagiaire() { BDDStagiaire?.SaveChanges(); }
        #endregion
    }
}
